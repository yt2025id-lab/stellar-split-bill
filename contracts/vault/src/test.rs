...
fn setup_vault(env: &Env) -> (Address, BillVaultClient<'_>, Address, Address) {
    env.mock_all_auths();
    ...
    (vault_id, client, factory_id, creator)
}
...
fn test_contribute_and_settle() {
    let env = Env::default();
    let (vault_id, client, factory, creator) = setup_vault(&env);
    ...
    client.init(&factory, &creator, &participants, &shares, &String::from_str(&env, "Test bill"), &100);
...
fn test_double_contribute_fails() {
    ...
    client.init(&factory, &creator, &participants, &shares, &String::from_str(&env, "Test"), &100);
...
fn test_partial_contribution() {
    ...
    client.init(&factory, &creator, &participants, &shares, &String::from_str(&env, "Group bill"), &300);
